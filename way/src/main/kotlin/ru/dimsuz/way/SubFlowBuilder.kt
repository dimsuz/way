...
class SubFlowBuilder<S : Any, R : Any, SR : Any> internal constructor() {
  private var flowNode: FlowNode<*, SR>? = null
  private var onResultTransition: ((ResultTransitionEnv<S, R, SR>) -> Unit)? = null

  enum class Error {
    MissingFlowNode
  }

  fun of(flow: FlowNode<*, SR>): SubFlowBuilder<S, R, SR> {
    flowNode = flow
    return this
  }

  fun onFinish(transition: ResultTransitionEnv<S, R, SR>.() -> Unit): SubFlowBuilder<S, R, SR> {
